...
export const ExitIntent = (options = {}) => {
  ...
  return (function() {
    ...
    const addEvent = (eventName: any, callback: any) => {
    ...
    const shouldDisplay = (position: any) => {
    ...
    const mouseDidMove = (event: any) => {
      if (shouldDisplay(event.clientY)) {
        if (Cookie.get(exitIntentModalSeenCookieString) !== 'true') {
          config.onExitIntent();
        }
    ...
    if (isMobileDevice()) {
      addEvent('scroll', throttle(onScroll, config.eventThrottle));
    }
